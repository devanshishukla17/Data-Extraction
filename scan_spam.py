...
class DataExtractor:
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                if not page_text.strip():
                    ...
                    page_text = pytesseract.image_to_string(img, config='--psm 4')  # best for paragraphs
                text += page_text + "\n"
            doc.close()
            return text
    ...
    def clean_extracted_value(self, value, field_name):
        ...
        if field_name == 'Name of the Patient':
            return ' '.join(value.split()) if len(value) > 2 else None
            ...
            return re.findall(r'\d+', value)[0] if re.findall(r'\d+', value) else None
        elif field_name == 'Policy Period':
            value = re.sub(r'-\s*\n\s*', '-', value)
            return re.sub(r'\s+', ' ', value).strip()
        elif field_name == 'Remarks':
            value = re.sub(r'\s+', ' ', value)
            return re.sub(r'^(Remarks?\s*:?\s*)', '', value, flags=re.IGNORECASE).strip()
