...
class DataExtractor:
    def __init__(self):
        self.patterns = {
            ...
            'Date of Admission': [
                r'Date\s+of\s+Admission\s*:?\s*(\d{2}[-­][A-Z]{3}[-­]\d{2,4})'
            ],
            'Date of Discharge': [
                r'Date\s+of\s+Discharge\s*:?\s*(\d{2}[-­][A-Z]{3}[-­]\d{2,4})',
                r'Discharge\s+Date\s*:?\s*(\d{2}[-­][A-Z]{3}[-­]\d{2,4})'
            ],
            'Name of the Patient': [
    ...
    def extract_policy_period(self, text):
        ...
        for line in lines:
            ...
            if capture:
                ...
                match = re.search(r'(\d{2}[-­][A-Z]{3}[-­]\d{2,4})\s+To\s+(\d{2}[-­][A-Z]{3}[-­]\d{2,4})', combined, re.IGNORECASE)
                if match:
    ...
    def extract_field(self, text, field_name):
        if field_name == 'Policy Period':
            value = self.extract_policy_period(text)
            return self.clean_extracted_value(value, field_name)
