...
class DataExtractor:
    def __init__(self):
        self.patterns = {
            ...
            'Policy Period': [
                ...
                r'Policy\s+Term\s*:?\s*([^:\n]*?To\s+[^:\n]*?(?:2025|25|26))',
                r'Policy\s+Period\s*:?\s*(\d{2}-[A-Z]{3}-\d{4}\s+To\s+\d{2}-[A-Z]{3}-\d{4})'
            ],
            'Total Bill Amount': [
    ...
    def clean_extracted_value(self, value, field_name):
        ...
        if field_name == 'Name of the Patient':
            ...
            # Clean up the policy period value
            value = re.sub(r'\s+', ' ', value)
            value = re.sub(r'(?i)Policy\s+Period\s*:\s*', '', value)
            value = re.sub(r'\s+To\s+', ' To ', value)
            return value.strip()
